...
#### LIDAR Raycasting Implementation Details

The LIDAR point cloud is generated by casting rays from the sensor position through the scene. This section documents critical implementation details to avoid common bugs.

**Coordinate System Flow:**
1. Ray directions are computed in **sensor-local space** using spherical coordinates (ROS convention: +X forward, +Y left, +Z up)
2. Directions are transformed to **Three.js world space** via `sensorWorldQuaternion` (includes rosRoot transform)
3. Raycasting happens in **Three.js world space** (Y-up)
4. Hit points are transformed back to **rosRoot local space** via `worldToLocal()` for storage in the point cloud geometry
5. Point cloud renders correctly because it's a child of rosRoot

**Ray Direction Formula (sensor-local space):**
```
...
- **Multi-channel LIDARs** (channels > 1): Distribute evenly from `-vFov/2` to `+vFov/2`
- **Single-channel LIDARs** (channels = 1): Always use `vAngle = 0` (horizontal scan)

**Critical: Raycast Target Filtering**
Only `THREE.Mesh` objects should be included in raycasting. Three.js raycaster can also intersect `LineSegments` (like GridHelper, AxesHelper) with a default threshold of **1 meter**, which causes grid-pattern artifacts if not filtered out.

```typescript
// CORRECT: Only collect Mesh objects
obj.traverse((child) => {
...
// WRONG: Including all objects causes GridHelper lines to register as hits
const raycastTargets = scenarioObjects.filter(obj => ...); // Don't do this
```
